      ...
      mongoose.connect(dbString, function(err) {
        if (err) {
          ...
          db.check_stats(settings.coin, function(exists) {
            if (exists == false) {
              console.log(
                "Run 'npm start' to create database structures before running this script."
              );
              exit();
            } else {
                              ...
                              function() {
                                db.update_richlist("received", function() {
                                  db.update_richlist("balance", function() {
                                    db.get_stats(settings.coin, function(
                                      nstats
                                    ) {
                                      console.log(
                                        "reindex complete (block: %s)",
                                        nstats.last
                                      );
                                      exit();
                                    });
          ...
          for (var x = 0; x < markets.length; x++) {
            ...
            db.check_market(market, function(mkt, exists) {
              if (exists) {
                ...
                console.log(
                  "error: entry for %s does not exists in markets db.",
                  mkt
                );
                complete++;
                if (complete == markets.length) {
