...
(exports.ex_title = "Block Explorer"),
  ...
  (exports.initial_index_alert =
    "Indexing is currently incomplete, functionality is limited until index is up-to-date."),
  ...
  (exports.reloadLocale = function reloadLocale(locale) {
    ...
    try {
      if (localeStr) {
        localeStr = jsonminify(localeStr)
          .replace(",]", "]")
          .replace(",}", "}");
      ...
      console.error(
        "There was an error processing your locale file: " + e.message
      );
      process.exit(1);
    }
    ...
    for (var i in lsettings) {
      ...
      //this setting is unkown, output a warning and throw it away
        ...
        console.warn(
          "Unknown Setting: '" +
            i +
            "'. This setting doesn't exist or it was removed"
        );
      }
    }
  });
